...
!!
!! @sub s_inverse_hilbert_d
!!
!! build an inverse of Hilbert matrix.
!! uses analytic formula for exact computation.
!!
...
!! [body
     ...
     ! build inverse Hilbert matrix using analytic formula:
     ! H_inv(i,j) = (-1)^(i+j) * (i+j-1) * C(n+i-1,n-j) * C(n+j-1,n-i) * C(i+j-2,i-1)^2
     do i=1,n
         do j=1,n
             ij = i + j
             ij_2 = i + j - 2
             ...
             ! compute term = C(n+i-1, n-j)
             term = one
             do k=n_j+1, n_i
                 term = term * real(k, dp) / real(k - n_j, dp)
             enddo

             ! compute C(n+j-1, n-i) and multiply
             do l=n_i+1, n_j
                 term = term * real(l, dp) / real(l - n_i, dp)
             enddo

             ! compute C(i+j-2, i-1)^2 and multiply
             hinv = one
             do k=i, ij_2
                 hinv = hinv * real(k, dp)
             enddo
             ...
             if ( mod(ij, 2) == 1 ) then
                 ...
                 A(i,j) = hinv
             endif ! back if ( mod(ij, 2) == 1 ) block
         enddo ! over j={1,n} loop
     enddo ! over i={1,n} loop
